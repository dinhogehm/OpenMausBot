...
```bash
pnpm dev:desktop   # starts the server and Vite, then opens the Electron shell
```

To work in a browser instead of the desktop shell, run `pnpm dev:server` and `pnpm dev` in two terminals and open the address Vite prints.
