...
const wireBot = (bot: BotRecord): WireBot => {
  ...
  return {
    ...visible,
    ...(activeCoordinationForThread(bot.threadId) && !visible.busy ? { busy: true, activity: "working" as const } : {}),
    avatarUrl: visible.avatarUrl ?? null,
    // Explicit false values clear stale permissions when clients merge frames.
    canMerge: visible.canMerge === true,
    canDeploy: visible.canDeploy === true,
...
const wireTrustedApprovalBot = (bot: NonNullable<ReturnType<typeof store.bot>>) => {
  ...
  return {
    ...rest,
    approvalMode: approvalModeFor(rest),
...
const approvalModeForTurn = (bot: BotRecord, peerInitiated = false, threadId: string | undefined = bot.threadId): ApprovalMode => {
...
function isUnattendedTurn(botId: string | null | undefined, threadId: string | undefined): boolean {
  return unattendedByEither(threadId === undefined ? undefined : turnProvenanceByThread.get(threadId), isUnattended(botId, threadId));
}
...
bus.subscribe((event: RuntimeEvent) => {
  ...
  // The engine only acts on turn.completed, so a private image reaching it
  // is inert — keep the call unconditional rather than widen the guard.
  ...
  switch (event.type) {
    ...
    case "request.opened": {
      ...
      // that downgrade's provenance on the card and audit row without
      // restoring Auto or replacing a more specific guard/remembered grant.
      if (verdict?.source === "no-grant" && unattended && asker && approvalModeFor(asker) === "auto") {
          ...
          if (outcome !== "allowed-once") {
            ...
            if (outcome === "rejected") {
              pushMessage({
                role: "bot",
                kind: "activity",
                tool: { name: "The provider rejected this action despite the automatic approval.", ok: false },
              });
              return;
            }
      ...
      const message = pushMessage({
        ...
        card: {
          ...
          allowKey: permission
            ...
            : undefined,
          // and, alongside it, the allow the PROVIDER can keep for its own
          // session — a different promise from the app's remembered grant,
          // so the card offers both rather than one standing for the other
...
async function startTurn(
  ...
  if (provenance.unattended) markUnattended(bot.id, threadId);
...
const webhooks = new WebhookManager({
  ...
  // Routine webhooks keep a delivery receipt, so a redelivery after a restart
  // finds the run it already started instead of starting a second one. A
  // workflow-routed delivery has no receipt of its own yet, and falls back to
  // the pre-existing behaviour rather than to a wrong answer.
    ...
    if (method === "PATCH" && instancePatch) {
      ...
      try {
        ...
        if (body.tools !== undefined) {
          if (!["openai-compat", "openrouter", "grok", "minimax"].includes(entry.driver)) {
            return json(res, 400, { error: "The tools setting is available for OpenAI-compatible, OpenRouter, Grok API and MiniMax API instances only." });
          }
