...
import { WebhookManager, type WebhookEnqueueInput } from "./webhooks.ts";

/** The app routes a delivery to a routine OR to a workflow run; this
 * harness only stands up the RoutineManager, so a workflow-routed input
 * would have nowhere to go. Narrow explicitly rather than casting. */
function routineDelivery(event: WebhookEnqueueInput) {
  if (event.botId === undefined) throw new Error("this harness only routes bot webhooks");
  return event;
}
...
it("recovers the canonical job when saving the ingress receipt failed, even with a full queue", () => {
  ...
  const options = {
    ...
    enqueue: (event: WebhookEnqueueInput) => {
      h.manager.enqueueWebhook(routineDelivery(event));
  ...
  const retried = new WebhookManager({
    ...options, enqueue: (event) => recovered.enqueueWebhook(routineDelivery(event)),
