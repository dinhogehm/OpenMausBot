...
export class WorkflowEngine {
  ...
  private sweepApprovals(now: number): void {
    for (const stale of this.store.listRuns()) {
      ...
      if (run.approvalRemindedAt === undefined && now >= run.approvalRequestedAt + windowMs / 2) {
        ...
        // reminder for good.
        if (this.announce(run, "reminder", `still needs approval at node "${node.id}" (reminder): ${node.prompt}`)) {
  ...
  private dispatchNode(runId: string, nodeId: string, botId?: string): void {
    ...
    if (node.kind === "approval") {
      this.openApproval(runId, node, entry);
      return;
    }
    if (node.kind === "notify") {
  ...
  private openApproval(runId: string, node: ApprovalNode, entry: NodeEntry): void {
    const patched = this.store.patchRun(runId, {
      ...entry,
    ...
    if (!patched) return;
    this.announce(patched, "approval", `needs approval at node "${node.id}": ${node.prompt}`);
  }
