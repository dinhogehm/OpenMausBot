...
export function stuckThresholdMs(workflow: Workflow, run: WorkflowRun, node: WorkflowNode | undefined): number | null {
  if (run.status === "waiting-approval") {
    const hours = node?.kind === "approval" ? (node.expiresHours ?? WORKFLOW_APPROVAL_EXPIRES_DEFAULT_H) : WORKFLOW_APPROVAL_EXPIRES_DEFAULT_H;
    return hours * WORKFLOW_STUCK_APPROVAL_FACTOR * 3_600_000;
  }
