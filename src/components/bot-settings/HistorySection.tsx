...
export function HistorySection({
  ...
  if (!rows) {
    return <div className="text-[13px] text-ink-secondary">{t("botHistory.loading")}</div>;
  }
  if (rows.length === 0) {
    return <div className="text-[13px] text-ink-secondary">{t("botHistory.empty")}</div>;
  }
  ...
  return (
    <div className="flex flex-col gap-2">
      {refreshError && (
        <div className="rounded-lg bg-inset px-3 py-2 text-[12.5px] text-ink-secondary">{t("botHistory.refreshError")}</div>
      )}
      {sorted.map((row) => (
        <div key={`${bot.id}-${row.id}`} className="rounded-xl bg-card p-4">
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0 flex-1 text-[13px] leading-relaxed text-ink">
              ...
              {" · "}
              <span>{t("botHistory.via", { actor: row.actor, via: row.via })}</span>
              {" · "}
          ...
          {row.field === "soul" && row.canRestore !== true && (
            <p className="mt-2 text-[12px] leading-relaxed text-ink-secondary">
              {row.restoreUnavailableReason ?? t("botHistory.undoUnavailable")}
            </p>
          )}
        </div>
      ))}
