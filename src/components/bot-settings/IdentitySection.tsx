...
export function IdentitySection({
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <Field label={t("botIdentity.name")}>
        <input
          className={inputCls}
      ...
      <Field label={t("botIdentity.title")}>
        <input
          className={inputCls}
          maxLength={BOT_PROFILE_LIMITS.title}
          placeholder={t("botIdentity.titlePlaceholder")}
          value={bot.title}
      ...
      <div className="block">
        <div className="mb-1.5 flex items-center justify-between gap-3">
          ...
          >
            <BookOpen size={12} /> {t("botIdentity.viewFull")}
          </button>
        </div>
        <textarea
          ...
          placeholder={t("botIdentity.blurbPlaceholder")}
          aria-label={t("botIdentity.blurb")}
          value={bot.description}
          onChange={(e) => patch({ description: e.target.value })}
        />
        <div className="mt-1.5 flex items-start justify-between gap-3 text-[11px] text-ink-secondary">
          <span>{t("botIdentity.blurbHint")}</span>
