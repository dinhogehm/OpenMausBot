...
export function PromptPreview({
  ...
  return (
    <div className="rounded-xl bg-card p-4">
      ...
      {open && !data && error && (
        <div className="mt-3 text-[13px] text-ink-secondary">{t("promptPreview.error")}</div>
      )}

      {open && !data && !error && <div className="mt-3 text-[13px] text-ink-secondary">{t("promptPreview.loading")}</div>}
          ...
          {data.sections.map((section) => (
            <details key={section.id} className="rounded-lg border border-hairline/40 bg-inset px-3 py-2">
              <summary className="flex cursor-pointer items-center justify-between gap-3 text-[13px] text-ink">
                ...
                <span className="shrink-0 tabular-nums text-ink-secondary">{t("promptPreview.bytes", { bytes: section.bytes.toLocaleString() })}</span>
              </summary>
